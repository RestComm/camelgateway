...
. Upload fact model.
+
To upload a model, use domain-x.y.z.jar which has the fact model (Call.java API) that you will be using in your rules.
...
+
Browse in newly open window and point to [path]_${JBOSS.HOME}/server/default/deploy/mobicents-ussd-gateway/lib/ussdgateway-domain-x.y.z.jar for JBoss AS or [path]_${JBOSS.HOME}/modules/system/layers/base/org/restcomm/camel/bootstrap/main/domain-x.y.z.jar for Wildfly 10
						_						. 
